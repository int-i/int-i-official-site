...
export const GetAllQuestions = async (req, res) => {
	try {
		...
		console.log(
			"404: Cannot get the page of showing all questions (GetAllQuestions in questionController) ",
			error
		);
...
export const GetOneQuestion = async (req, res) => {
	try {
		// 요청으로 오는 id는 string값이므로 ObjectId로 변환시켜줘야한다
		const objectId = mongoose.Types.ObjectId(`${req.params.id}`);
		const question = await Question.findById({ _id: objectId });
		return res.status(200).send(question);
	} catch (err) {
		console.log(
			"404: Cannot get page of showing specific one question (GetOneQuestion in questionController) ",
			err
		);
		next(err);
	}
...
export const PostEditPost = async (req, res, next) => {
	...
	try {
		...
		if (checkauthor.author !== user.nickname) {
			console.log(
				"403: This user does not have authority to edit question. (PostEditPost in questionController) "
			);
			return res
				.status(403)
				.json({ updateQuestion: false, reason: "only author of the post has authority to edit." });
		}

		// 자동으로 생성된 아이디로 게시글을 찾고 업데이트 시켜준다.
		const rawData = await Question.findByIdAndUpdate(_id, {
			$set: { title: title, contents: contents, anonymous: anonymous, createdAt: createdAt },
		});
		res.locals.schema = Question;
...
export const PostDeleteQuestion = async (req, res, next) => {
	...
	try {
		...
		if (checkauthor.author !== user.nickname) {
			console.log(
				"403: This user does not have authority to delete question. (PostDeleteQuestion in questionController) "
			);
			return res
				.status(403)
				.json({ deleteQuestion: false, reason: "only author of the post has authority to delete." });
		}
